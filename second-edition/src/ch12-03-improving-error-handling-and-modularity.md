...
previously used the separate `query` and `filename` variables so that is now
...
In order to handle the error case and print a user-friendly message, we need
to update `main` to handle the `Result` being returned by `Config::new`, as
shown in Listing 12-10. We’re also going to take the responsibility of exiting
the command line tool with a nonzero error code from `panic!` and implement it
by hand. A nonzero exit status is a convention to signal to the process that
...
we used in Listing 12- 8, with the exception that we no longer get all the
...
### Extracting Logic Functionality from `main`
...
use greprs::Config line to bring the Config type into scope, and prefix the run 
